...
function lista(respostas){
    ...
    respostas.forEach(element => {
            ...
            html+="<td><button class='btn btn-success' onclick="+"editar("+element['id_entra']+")"+">&#9850;</button></td>";
            html+='</tr>';
    }); 
    html+='</tbody></table>';
...
$(document).ready(function(){
    ...
    $.ajax({
        method: 'POST',
        url: "<?=$path?>/situacao/filtrar_entradas",
...
function editar(id)
{
   $.ajax({
       method:'post',
       url:"<?=$path?>/situacao/filtrar_id/"+id,
       dataType:'json',
       success:function(resposta){
...
function salvar()
{
    ...
    $.ajax({
        method:'post',
        url:'<?=$path?>/situacao/insert_entradas',
        ...
        $.ajax({
        method:'post',
        url:'<?=$path?>/situacao/update_entradas',
